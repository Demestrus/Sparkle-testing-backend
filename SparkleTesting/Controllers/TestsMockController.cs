...
namespace SparkleTesting.API.Controllers
{
    ...
    [Authorize]
    [ApiVersion("0.5")]
    public class TestsMocksController : ControllerBase
    {
        public TestsMocksController()
        {
        ...
        public IEnumerable<TestDto> GetTests()
        {
            return new List<TestDto>()
            {
                new TestDto()
                {
                    Id = 1,
                    TestInProgress = true,
                    TestName = "Первый Mock-тест!",
                    TestAttemptTime = TimeSpan.FromMinutes(15).ToString()
                },
                new TestDto()
                {
                    Id = 2,
                    TestInProgress = false,
                    TestName = "Второй Mock-тест!",
                    TestAttemptTime = TimeSpan.FromMinutes(15).ToString()
                },
                new TestDto()
                {
                    Id = 3,
                    TestInProgress = true,
                    TestName = "Третий все я устал мокать...",
                    TestAttemptTime = TimeSpan.FromMinutes(15).ToString()
                }
            };
        }
